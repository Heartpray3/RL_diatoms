...
if __name__ ==  '__main__':
  ...
  parser.add_argument('--To_step', type = int)
  parser.add_argument('--power_swim_arr', type = float)
  ...
  power_swim_arr = args.power_swim_arr
  ...
  Rfus = []
  forces = []
  for n in range(From_step, To_step):
      bodies = []
      body_types = []
      body_names = []
      blobs_offset = 0
      for ID, structure in enumerate(read.structures):
        ...
        name = structure[0].replace('x.', str(n)+'.')
        struct_ref_config = read_vertex_file.read_vertex_file(name)
      ...
      if read.velocity_file is not None:
        name = read.velocity_file.replace('step_x','step_'+str(n))
        print(name)
        velocity = np.loadtxt(name)
      ...
      # If scheme == resistance solve resistance problem 
        ...
        resistance_bodies2 = np.linalg.pinv(mobility_bodies)
        
        Rfu = resistance_bodies[0:3,0:3]
        vel = velocity[0]
    
        # Save Velocity
        # name = read.output_name + '.vel.dat'
        # np.savetxt(name, vel, delimiter='  ')
        vels.append(vel)
        ...
        # Save R_FU 
        # name = read.output_name + '.RFU.dat'  
        # np.savetxt(name, Rfu, delimiter='  ')
        Rfus.append(Rfu)
      ...
      Rfus_mean = np.mean(Rfus,axis=0)
      V_tilde = np.mean(vels, axis = 0)
      V_tilde[1] = 0
      forces = Rfus @ V_tilde
      F_mean = np.mean(forces,axis = 0)
      power_pull_arr = np.dot(F_mean,V_tilde)
      efficiency = power_pull_arr/power_swim_arr
      
      fichier = open('Efficiency', 'w')
